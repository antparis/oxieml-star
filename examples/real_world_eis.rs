//! REAL-WORLD TEST: Electrochemical Impedance Spectroscopy
//!
//! Uses published Randles circuit parameters (Barsoukov & Macdonald, 2005)
//! to test eml_star on real-world battery impedance data.
//!
//! Usage: cargo run --example real_world_eis
...
fn main() {
    println!("===================================================================");
    println!("  REAL-WORLD TEST: Battery Impedance Spectroscopy");
    println!("  Randles circuit (Barsoukov & Macdonald, Wiley 2005)");
    println!("  R_s=0.05 ohm, R_ct=0.15 ohm, C_dl=0.001 F");
    println!("===================================================================\n");

    let content = fs::read_to_string("examples/real_eis_data.txt")
        .expect("Cannot read real_eis_data.txt");
    ...
    // Step 1: Verify |Z|^2 via Theorem 3.1
    println!("--- Step 1: Verify |Z|^2 = Z * conj(Z) via eml_star ---");
    let mut thm_mse = 0.0;
    ...
    println!("|Z|^2 via eml_star: MSE = {:.6e} [{}]\n", thm_mse, status);

    // Step 2: Blind discovery — can the engine find |Z|^2 from Z?
    println!("--- Step 2: Blind discovery on impedance data ---");
    let max_depth = 3;
    ...
    println!("Top 10 discovered formulas:");
    println!("{:-<80}", "");
    ...
    let star_in_top = results.iter().take(10).filter(|r| r.2).count();
    println!("\n  eml_star in top 10: {}/10", star_in_top);

    println!("\n===================================================================");
    println!("  Real-world EIS test complete");
    println!("===================================================================");
}
