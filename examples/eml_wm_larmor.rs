//! EML-WM Prototype: Larmor Precession
//!
//! Tests whether oxieml-star can rediscover Re(psi) = (psi + conj(psi))/2
//! from spin-1/2 precession data. This requires eml★ for conjugation.
//!
//! Usage: cargo run --example eml_wm_larmor
...
fn has_eml_star(node: &EmlNode) -> bool {
    match node {
        ...
        EmlNode::Eml { left, right } | EmlNode::EmlStar { left, right } => {
            has_eml_star(left) || has_eml_star(right)
        }
    }
}

fn main() {
    println!("=== EML-WM Prototype: Larmor Precession ===");
    println!("Target: Re(psi) = (psi + conj(psi))/2\n");

    // Load data
    let content = fs::read_to_string("examples/larmor_data.txt")
        .expect("Cannot read larmor_data.txt");
    ...
    // Verify Theorem 3.1 on this data
    println!("=== Theorem 3.1 on Larmor data ===");
    let mut thm_mse = 0.0;
    for (inputs, target) in &data {
        let z = inputs[0];
        ...
        // Re(z) = (z + conj(z)) / 2
        let re_z = (z + conj_z) * Complex64::new(0.5, 0.0);
        let err = re_z - target;
        thm_mse += err.re * err.re + err.im * err.im;
    }
    thm_mse /= data.len() as f64;
    println!("Re(z) via eml_star: MSE = {:.6e}", thm_mse);
    if thm_mse < 1e-20 {
    ...
    let star_in_top5 = results.iter().take(5).filter(|r| r.2).count();
    println!("\neml_star in top 5: {}/5", star_in_top5);
    println!("\n=== EML-WM: World model uses eml_star for anti-holomorphic prediction ===");
}
