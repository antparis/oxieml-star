//! EML-WM: Quantum probability density |psi|^2 = psi * conj(psi)
//!
//! Tests whether oxieml-star can verify |psi|^2 via eml_star on
//! a non-trivial superposition psi = cos(t) + i*sin(2t).
//!
//! Usage: cargo run --example eml_wm_probability
...
fn main() {
    println!("=== EML-WM: Quantum Probability Density ===");
    println!("Target: |psi|^2 = psi * conj(psi)");
    println!("System: psi = cos(t) + i*sin(2t)\n");

    let content = fs::read_to_string("examples/probability_data.txt")
        .expect("Cannot read probability_data.txt");
    ...
    println!("=== Theorem 3.1: |psi|^2 via eml_star ===");
    let mut thm_mse = 0.0;
    for (inputs, target) in &data {
        let z = inputs[0];
        let one = Complex64::new(1.0, 0.0);
        let zero = Complex64::new(0.0, 0.0);
        // conj(z) via Theorem 3.1
        let inner = z.exp();
    ...
    println!("|psi|^2 via eml_star: MSE = {:.6e}", thm_mse);
    if thm_mse < 1e-20 {
