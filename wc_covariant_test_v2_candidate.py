#!/usr/bin/env python3
# wc_covariant_test_v2_candidate.py -- complex-domain recertification candidate
#
...
#
#   JD  domain: c1 and c2 are explicitly unrestricted nonzero complex
#       symbols; they are not silently restricted to the real line.
...
EXPECTED_CLAUSES = 14
EXPECTED_IDS = (
    "JD", "J0", "J1a", "J1b", "J1c", "J2", "J3", "J4a", "J4b",
    "J5a", "J5b", "J5c", "J6a", "J6b",
)
...
# ---------------------------------------------------------- symbols
c1, c2 = sp.symbols('c1 c2', complex=True, zero=False)  # complex, nonzero
rho    = sp.Symbol('rho', positive=True)              # size deviation
epsp   = sp.Symbol('epsilon', complex=True, zero=False)  # K_eps knob
...
print("=" * 72)
print("W_C COVARIANT WITNESS -- complex-domain recertification candidate")
print("=" * 72)

# ------------------------------------------------------------ domain
jd = all(
    symbol.is_complex is True
    and symbol.is_real is None
    and symbol.is_zero is False
    for symbol in (c1, c2)
)
clause("JD  c1,c2 are unrestricted nonzero complex symbols", jd)
...
a, b, d = sp.symbols('a b d', complex=True, zero=False)
...
p, q = sp.symbols('p q', complex=True, zero=False)
...
observed_ids = tuple(name.split(maxsplit=1)[0] for name, _, _ in results)
integrity_ok = (
    len(results) == EXPECTED_CLAUSES
    and observed_ids == EXPECTED_IDS
)
if not integrity_ok:
    print(
        "FAIL-CLOSED: "
        f"expected {EXPECTED_CLAUSES} clauses with IDs {EXPECTED_IDS}; "
        f"got {len(results)} clauses with IDs {observed_ids}"
    )
print(f"VERDICT: {npass}/{EXPECTED_CLAUSES} clauses PASS")
for name, ok, _ in results:
    if not ok:
        print(f"  FAILED: {name}")
print("Status target: candidate for #063 complex-domain recertification "
      "iff all PASS on Anthony's machine and the run is sealed.")
sys.exit(0 if integrity_ok and npass == EXPECTED_CLAUSES else 1)
