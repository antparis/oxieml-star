...
\begin{abstract}
We introduce $\emlstar(x,y) = \exp(x) - \ln(\conj{y})$,
...
where $\emlstar$ recovers the modulus structure $|\gamma|^2$ on
5\,000 real complex measurements as a sanity check on observational data.
...
\begin{equation}
\emlstar(x,y) = \exp(x) - \ln(\conj{y}), \quad y \neq 0.
\label{eq:emlstar}
\end{equation}
\end{definition}
...
\emlstar(0, \exp(z)) &= \exp(0) - \ln(\conj{\exp(z)}) \\
  &= 1 - \ln(\exp(\conj{z})) \\
  &= 1 - \conj{z},
\end{align}
...
PySR recovers the expected modulus structure on natively complex data.
...
and quantum evolution (7/7 classification), with a sanity check
on real complex astrophysical data (KiDS-1000) and rigorous
negative controls confirming specificity.
