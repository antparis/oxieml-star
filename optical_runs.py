...
# Clone of vortex_N1_controls.py, targeted at the optical vortex CSVs.
# Same ks.run_one (precision=64, MIXTE+inv+inv_bar, anti-overfit brakes) as N1.
# Datasets: anti=conj(z)^3, holo=z^3, shuf=shuffled anti (negative control).
...
def main():
    ...
    ap.add_argument("--only", choices=["all", "anti", "holo", "shuf"], default="all")
    ap.add_argument("--out_dir", default=".")
    ap.add_argument("--quiet", action="store_true")
    args = ap.parse_args()

    datasets = [
        ("optical_anti", "optical_anti.csv"),
        ("optical_holo", "optical_holo.csv"),
        ("optical_shuf", "optical_shuf.csv"),
    ]
    if args.only != "all":
