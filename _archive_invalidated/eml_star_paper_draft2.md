...
We list the best-fit formulas containing eml★ for five galaxies with high improvement (Δ > 70%). Variables: x0 = radius r, x1 = baryonic velocity V_bar (when present). Operators: emlstar_re = eml★, emlstar_self = eml★(z,z), double_emlstar = eml★(eml★(z,w),w).
...
In all five cases, eml★ operators appear in nested positions within the formula tree, indicating that the anti-holomorphic contribution is not additive but multiplicatively coupled to the holomorphic structure. The constants have been rounded to two decimal places for readability; full-precision values are available in the supplementary data.
