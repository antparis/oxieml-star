...
# ============================================================
DATA_DIR = "data"
OUTPUT_JSON = "double_validation_v3_result.json"

DATASETS = {
    "mandelbrot":      "fractal_mandelbrot.csv",  # z -> z^2 + c                  (holo)
    "tricorn":         "fractal_tricorn.csv",     # z -> conj(z)^2 + c            (anti)
    "tricorn_layer1":  "layer_tricorn_1.csv",     # z -> conj(z)^2 + c            (anti)
    "tricorn_layer2":  "layer_tricorn_2.csv",     # z -> conj(conj(z)^2+c)^2+c    (anti, nested)
    "wm_larmor":       "wm_larmor.csv",           # Larmor precession             (holo)
    "wm_rabi":         "wm_rabi.csv",             # Rabi oscillation              (holo)
}

# Expected verdict per dataset, all verified algebraically against known formulas.
EXPECTED = {
    "mandelbrot":     "holo",
    "tricorn":        "anti",
    "tricorn_layer1": "anti",
