...
v2 improvements over v1:
  - Incremental save: JSON written after EVERY run, not only at the end.
    If the process crashes at run 3/4, runs 1 and 2 are preserved on disk.
  - Visible progress: [n/total] dataset - toolbox, flushed in real time.
  - Resume: if a partial JSON exists, already-completed runs are skipped.

Operator definitions are copied verbatim from pysr_stacking.py to guarantee
consistency with the published paper (v8).

Usage:
    python3 -u double_validation_v2.py             # run all
    python3 -u double_validation_v2.py --resume    # skip completed runs
    python3 -u double_validation_v2.py --datasets mandelbrot tricorn
...
# ============================================================
DATA_DIR = "data"
OUTPUT_JSON = "double_validation_v5b_kids_fast_result.json"

DATASETS = {
    "kids_abs2": "kids_abs2_test.csv",  # |gamma|^2 from 5000 real KiDS galaxies (anti)
}

# Expected verdict, verified algebraically: target = |z|^2 = z*conj(z) (anti).
EXPECTED = {
    "kids_abs2": "anti",
}
...
# progress bar during the search (so long runs are not a black box).
PYSR_BASE = dict(
    niterations=100,
    population_size=50,
    precision=64,
...
# Subsample size for large real datasets (KiDS has 5000 points; 500 is plenty
# to detect the |gamma|^2 structure and runs ~10x faster). Seeded for repro.
SUBSAMPLE = 500
SUBSAMPLE_SEED = 42
...
def load_dataset(csv_name):
    ...
    # Accept both column conventions: z_real/z_imag or z_re/z_im.
    if "z_real" in df.columns:
        zr, zi, tr, ti = "z_real", "z_imag", "target_real", "target_imag"
    elif "z_re" in df.columns:
        zr, zi, tr, ti = "z_re", "z_im", "target_re", "target_im"
    else:
        raise ValueError(f"Unrecognized columns in {csv_name}: {list(df.columns)}")
    # Subsample large datasets for speed (seeded, reproducible).
    if len(df) > SUBSAMPLE:
        df = df.sample(n=SUBSAMPLE, random_state=SUBSAMPLE_SEED).reset_index(drop=True)
        log(f"  [subsampled to {SUBSAMPLE} points, seed={SUBSAMPLE_SEED}]")
    z = df[zr].values + 1j * df[zi].values
    y = df[tr].values + 1j * df[ti].values
...
def main():
    ...
    for ds_name in args.datasets:
        ...
        if a is None or b is None:
            log("{:<14}{:<10}{}".format(ds_name, exp, "incomplete"))
            continue
        ...
        log("{:<14}{:<10}{:<24}{:<24}".format(ds_name, exp, cell_a, cell_b))
    log("(' !' marks a verdict that disagrees with the physically expected one)")
